...
module.exports = grammar({
  ...
  rules: {
    program: ($) => optional(repeat($._statement)),

    php_tag: ($) => /<\?([pP][hH][pP]|=)?/,

    text_interpolation: ($) =>
      seq("?>", optional($.text), choice($.php_tag, $._eof)),

    text: ($) => repeat1(choice(token(prec(-1, /</)), /[^\s<][^<]*/)),
    ...
    global_declaration: ($) =>
      seq(keyword("global"), commaSep1($._variable_name), $._semicolon),
    ...
    namespace_use_declaration: ($) =>
      seq(
        ...
        choice(
          ...
          seq(optional("\\"), $.namespace_name, "\\", $.namespace_use_group)
        ),
        $._semicolon
      ),
    ...
    qualified_name: ($) => seq($.namespace_name_as_prefix, $.name),
    ...
    namespace_aliasing_clause: ($) => seq(keyword("as"), $.name),

    namespace_use_group: ($) =>
      seq("{", commaSep1($.namespace_use_group_clause), "}"),
    ...
    _enum_member_declaration: ($) =>
      choice($.enum_case, $.method_declaration, $.use_declaration),
    ...
    class_declaration: ($) =>
      prec.right(
        seq(
          optional(field("attributes", $.attribute_list)),
          optional(
            field("modifier", choice($.final_modifier, $.abstract_modifier))
          ),
          keyword("class"),
          field("name", $.name),
          optional($.base_clause),
    ...
    property_element: ($) =>
      seq($.variable_name, optional($.property_initializer)),

    property_initializer: ($) => seq("=", $._expression),
    ...
    use_list: ($) =>
      seq(
        "{",
        repeat(
          seq(choice($.use_instead_of_clause, $.use_as_clause), $._semicolon)
        ),
        "}"
      ),
    ...
    visibility_modifier: ($) =>
      choice(keyword("public"), keyword("protected"), keyword("private")),
    ...
    arrow_function: ($) =>
      seq($._arrow_function_header, "=>", field("body", $._expression)),
    ...
    _return_type: ($) =>
      seq(":", field("return_type", choice($._type, $.bottom_type))),
    ...
    unset_statement: ($) =>
      seq("unset", "(", commaSep1($._variable), ")", $._semicolon),
    ...
    declare_directive: ($) =>
      seq(choice("ticks", "encoding", "strict_types"), "=", $._literal),

    _literal: ($) => choice($.integer, $.float, $._string, $.boolean, $.null),
    ...
    finally_clause: ($) =>
      seq(keyword("finally"), field("body", $.compound_statement)),

    goto_statement: ($) => seq(keyword("goto"), $.name, $._semicolon),

    continue_statement: ($) =>
      seq(keyword("continue"), optional($._expression), $._semicolon),

    break_statement: ($) =>
      seq(keyword("break"), optional($._expression), $._semicolon),
    ...
    integer: ($) => {
      ...
      return token(choice(decimal, octal, hex, binary));
    },

    return_statement: ($) =>
      seq(keyword("return"), optional($._expression), $._semicolon),

    throw_expression: ($) => seq(keyword("throw"), $._expression),
    ...
    while_statement: ($) =>
      seq(
        ...
        choice(
          ...
          seq(field("body", $.colon_block), keyword("endwhile"), $._semicolon)
        )
      ),
    ...
    sequence_expression: ($) =>
      prec(
        PREC.COMMA,
        seq($._expression, ",", choice($.sequence_expression, $._expression))
      ),
    ...
    foreach_statement: ($) =>
      seq(
        ...
        choice(
          $._semicolon,
          field("body", $._statement),
          seq(field("body", $.colon_block), keyword("endforeach"), $._semicolon)
        )
      ),

    foreach_pair: ($) => seq($._expression, "=>", $._foreach_value),

    _foreach_value: ($) => choice($.by_ref, $._expression, $.list_literal),
      ...
      seq(
        ...
        choice(
          ...
          seq(
            ...
            repeat(
              field("alternative", alias($.else_if_clause_2, $.else_if_clause))
            ),
            optional(
    ...
    colon_block: ($) => seq(":", repeat($._statement)),
    ...
    else_clause: ($) => seq(keyword("else"), field("body", $._statement)),
    ...
    match_default_expression: ($) =>
      seq(keyword("default"), "=>", field("return_expression", $._expression)),
    ...
    default_statement: ($) =>
      seq(keyword("default"), choice(":", ";"), repeat($._statement)),
    ...
    clone_expression: ($) => seq(keyword("clone"), $._primary_expression),
    ...
    print_intrinsic: ($) => seq(keyword("print"), $._expression),
    ...
    anonymous_function_use_clause: ($) =>
      seq(
        ...
        "(",
        commaSep1(
          choice(alias($.variable_reference, $.by_ref), $.variable_name)
        ),
        optional(","),
        ")"
      ),
    ...
    object_creation_expression: ($) =>
      prec.right(
        PREC.NEW,
        choice(
          seq(keyword("new"), $._class_type_designator, optional($.arguments)),
          seq(
            keyword("new"),
            optional(field("attributes", $.attribute_list)),
            keyword("class"),
    ...
    shell_command_expression: ($) => token(seq("`", backtick_chars(), "`")),
    ...
    cast_variable: ($) =>
      prec(
        PREC.CAST,
        seq("(", field("type", $.cast_type), ")", field("value", $._variable))
      ),
    ...
    conditional_expression: ($) =>
      prec.left(
        PREC.TERNARY,
        seq(
          // TODO: Ternay is non-assossiative after PHP 8
    ...
    _array_destructing: ($) =>
      seq("[", commaSep1(optional($._array_destructing_element)), "]"),
    ...
    relative_scope: ($) =>
      prec(PREC.SCOPE, choice("self", "parent", keyword("static"))),
    ...
    attribute_group: ($) =>
      seq("#[", commaSep1($.attribute), optional(","), "]"),
    ...
    _complex_string_part: ($) => seq("{", $._expression, "}"),

    _simple_string_member_access_expression: ($) =>
      prec(
        PREC.MEMBER,
        seq(field("object", $.variable_name), "->", field("name", $.name))
      ),

    _simple_string_subscript_unary_expression: ($) =>
      prec.left(seq("-", $.integer)),
    ...
    _interpolated_string_body: ($) =>
      repeat1(
        choice(
          ...
          seq(
            $.variable_name,
            alias($.encapsed_string_chars_after_variable, $.string_value)
          ),
    ...
    encapsed_string: ($) =>
      prec.right(
        seq(choice(/[bB]"/, '"'), optional($._interpolated_string_body), '"')
      ),

    string: ($) => seq(choice(/[bB]'/, "'"), $.string_value, "'"),
    ...
    nowdoc_body: ($) =>
      seq(
        $._new_line,
        choice(repeat1($.nowdoc_string), alias("", $.nowdoc_string))
      ),
    ...
    dynamic_variable_name: ($) =>
      choice(seq("$", $._variable_name), seq("$", "{", $._expression, "}")),
    ...
    include_expression: ($) => seq(keyword("include"), $._expression),

    include_once_expression: ($) => seq(keyword("include_once"), $._expression),

    require_expression: ($) => seq(keyword("require"), $._expression),

    require_once_expression: ($) => seq(keyword("require_once"), $._expression),

    name: ($) => /[_a-zA-Z\u00A1-\u00ff][_a-zA-Z\u00A1-\u00ff\d]*/,

    _reserved_identifier: ($) => choice("self", "parent", keyword("static")),
...
function backtick_chars() {
  ...
  const dq_hex_escapes = seq(/\\[xX]/, hex_digit, optional(hex_digit));

  const dq_unicode_escapes = seq("\\u{", repeat1(hex_digit), "}");
  const dq_escapes = choice(
    dq_simple_escapes,
    dq_octal_escapes,
    dq_hex_escapes,
    dq_unicode_escapes
